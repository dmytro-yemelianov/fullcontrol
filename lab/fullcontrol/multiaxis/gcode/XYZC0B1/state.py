...
from lab.fullcontrol.multiaxis.gcode.XYZC0B1.printer import PrinterState
            ...
            if type(steps).__name__ == 'list':
                for i in range(len(steps)):
                    if type(steps[i]).__name__ == 'Point':
                        if fully_defined:
                            if steps[i].x is not None and steps[i].y is not None and steps[i].z is not None and steps[i].b is not None and steps[i].c is not None:
                                return steps[i]
                        else:
                            return steps[i]
            if fully_defined:
