...
def set_up(user_overrides: dict):
    ...
    # copy the toolchanger_T0 initialization data except select tool T3 instead of T0
    initialization_data = toolchanger_T0.set_up(user_overrides)
    replace_step(initialization_data['starting_procedure_steps'],
                 manual_gcode_text('T0'), ManualGcode(text='T3'), description='tool-select command')
